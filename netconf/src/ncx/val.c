...
if (strcmp((const char *)val->name,"get-time") == 0) {
    FILE *fp1;
    fp1 = fopen("/etc/yuma/get-time.txt", "w");
    ...
    case NCX_BT_LEAFREF:
        ...
        if (VAL_STR(val)) {
            ...
            if (val->obj && obj_is_password(val->obj)) {
               ...
               //////////////////////////////////
               // Start scheduled-time processing
               //////////////////////////////////
               if ( 1 == flag && LOGDEBUG3){
                    
                 char* date = (char*)VAL_STR(val);
                 struct timeval tv_curr_time;
                 struct tm brokentime_curr;
                 char buffer_curr[80], buffer[80];
                 struct tm wanted_time;
                 struct tm time_after_sleep;
                 struct timeval tv_after_sleep;

                 FILE *fp;
                 static int i=1;
                 fp = fopen("/etc/yuma/time_log.txt", "a");
                 fprintf(fp,"\n\nscheduled-time %d = %s", i, date);
                 i++;
                 fclose (fp);
                 
                 wanted_time.tm_sec  = (date[17]-'0')*10+(date[18]-'0');
                 wanted_time.tm_min  = (date[14]-'0')*10+(date[15]-'0');
                 wanted_time.tm_hour = (date[11]-'0')*10+(date[12]-'0');
                 wanted_time.tm_mday = (date[8]-'0')*10+(date[9]-'0');
                 wanted_time.tm_mon  = (date[5]-'0')*10+(date[6]-'0') -1 ;
                 wanted_time.tm_year = (date[0]-'0')*1000+(date[1]-'0')*100+(date[2]-'0')*10+(date[3]-'0') -1900;

             
                 int given_time_length=strlen(date);
                 int num_of_digits_after_sec=0;
                 int utc_hour=0;
                 if(date[given_time_length-1]=='Z' || date[given_time_length-1]=='z')
                 {
                      num_of_digits_after_sec = given_time_length - 21;
                      utc_hour=0;
                 }
                 else
                 {  
                      num_of_digits_after_sec = given_time_length - 26;
                      utc_hour=(date[given_time_length-5]-'0')*10+(date[given_time_length-4]-'0');
                      if (date[given_time_length-6]=='-')
                           utc_hour=0-utc_hour;
                 }
                 long int wanted_micro = 0;
                 if (num_of_digits_after_sec>6) num_of_digits_after_sec=6;
                 for (int j=0; j<num_of_digits_after_sec ; j++){
                     wanted_micro = wanted_micro + pow(10,num_of_digits_after_sec-1-j)*(date[20+j]-'0');  
                 } 

                gettimeofday (&tv_curr_time, NULL);
                localtime_r (&tv_curr_time.tv_sec, &brokentime_curr);
                strftime (buffer, sizeof(buffer), "%F  %z %T", &brokentime_curr);

                int server_utc=(buffer[13]-'0')*10+(buffer[14]-'0');
                if(buffer[12]=='-')
                     server_utc=0-server_utc;
                int delay_hour=server_utc-utc_hour;
                wanted_time.tm_hour=wanted_time.tm_hour+delay_hour;
                if(wanted_time.tm_hour>=24)
                   wanted_time.tm_hour=wanted_time.tm_hour-24;
                else if( wanted_time.tm_hour<0)
                   wanted_time.tm_hour=wanted_time.tm_hour+24;


                time_t curr_time, wan_time;
                //printf ("\n\nwanted_time.tm_isdst= %d\n" ,wanted_time.tm_isdst);
                wanted_time.tm_isdst=-1;
                wan_time = mktime(&wanted_time);
                //printf("\nwanted time sec = %llu\n" , wan_time);
                //printf("\ncurr time sec = %llu\n" , curr_time);


                curr_time=tv_curr_time.tv_sec;
                //printf("\ncurr time sec = %llu\n" , curr_time);
                double f_seconds =difftime(curr_time,wan_time);
                //printf("\ndifftime-seconds(double) = %f\n", f_seconds);
                int i_seconds=0-(int)f_seconds;
                //printf("\ndifftime-seconds(int) = %d\n", i_seconds);
                wanted_micro = wanted_micro -tv_curr_time.tv_usec ;
		long int total_sleep_in_micro = 1000000*i_seconds+wanted_micro ;
                //printf ("\ntotal_sleep_in_micro = %lu\n" , total_sleep_in_micro);
                while (total_sleep_in_micro>1000000){
                    //printf("\nsleeping for 1 sec\n");
                    usleep(999999);
                    total_sleep_in_micro = total_sleep_in_micro - 999999;
                }
                if (total_sleep_in_micro > 0) {
                    //printf("\nsleeping for %lu micro\n", total_sleep_in_micro);
                    usleep(total_sleep_in_micro);
                }
                ...
                FILE *fp2;
                gettimeofday(&tv_after_sleep, NULL);
                localtime_r (&tv_after_sleep.tv_sec, &time_after_sleep);
                                     
                fp2 = fopen("/etc/yuma/time_log.txt", "a");
                fprintf(fp2,"\nStarted %d = %d-%02d-%02dT%02d:%02d:%02d.%06lu%c%c%c:00 ", i-1,time_after_sleep.tm_year+1900,time_after_sleep.tm_mon+1,time_after_sleep.tm_mday,time_after_sleep.tm_hour,time_after_sleep.tm_min, time_after_sleep.tm_sec ,tv_after_sleep.tv_usec,buffer[12],buffer[13],buffer[14] );
                fclose (fp2);
                }
               /////////////////////////////////////
               // Finished scheduled-time processing
               /////////////////////////////////////
            }
            if (quotes) {
