                    ...
                    rpc_msg_t  *msg)
{
    ...
    /////////////////////////////////////////////
    // Start get-time processing
    /////////////////////////////////////////////
    FILE *fp1;
    int flag_get_time=0;
    if (fp1 = fopen("/etc/yuma/get-time.txt", "r")) {
      flag_get_time=1;
      fclose(fp1);
      unlink("/etc/yuma/get-time.txt");
    }
    ...
    if (flag_get_time == 1) {
        ...
        localtime_r (&tv4.tv_sec, &brokentime);

        /* Generate the majority of the time display format. */
        strftime (buffer, sizeof(buffer), "%F  %z %T", &brokentime);

        fprintf(fp,"\nexecution-time = %d-%02d-%02dT%02d:%02d:%02d.%06lu%c%c%c:00 ", brokentime.tm_year+1900,brokentime.tm_mon+1,brokentime.tm_mday,brokentime.tm_hour,brokentime.tm_min, brokentime.tm_sec ,tv4.tv_usec,buffer[12],buffer[13],buffer[14] );

        fclose(fp);

        fp = fopen("/etc/yuma/time_log.txt", "r");
        char*  x_string;
        char str[99];
        int length=0;
        ...
        char start_att[100]="<execution-time>";
        char end_att[100]="</execution-time>";
        strcat(start_att, x_string);
        strcat(start_att, end_att);
        ses_indent(scb, indent);
        ses_putstr(scb, start_att);	
...
void * initMessage(const char* msg) {
